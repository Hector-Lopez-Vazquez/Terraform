pipeline {
    ...
    steps {
        sh '''
            echo "=== Esperando que Flask responda dentro del contenedor ==="
            timeout 90 sh -c '
                until docker-compose -f docker-compose.test.yml exec -T test-web curl -s -f http://localhost:5000/login; do
